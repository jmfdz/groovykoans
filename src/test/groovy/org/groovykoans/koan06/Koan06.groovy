...
class Koan06 extends GroovyTestCase {
    ...
    void test02_CollectMethodOnLists() {
        ...
        def uniqueTypes = []
        // ------------ START EDITING HERE ----------------------
		def tempAllTypes = differentTypes.collect { it.class }
		uniqueTypes = tempAllTypes.unique()
    ...
    void test03_FileIteration() {
		...
		file.eachFileRecurse { File fich ->
			if(!fich.isDirectory() && fich.text.contains("Lorem"))
				count++ 
		}
