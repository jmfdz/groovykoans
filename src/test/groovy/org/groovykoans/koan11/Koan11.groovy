...
class Koan11 extends GroovyTestCase {
    ...
    void test02_MoreOnInsertingData() {
        ...
        Sql.withInstance('jdbc:h2:mem:groovyDb2', 'sa', '', 'org.h2.Driver') { db ->
            ...
            // ------------ START EDITING HERE ----------------------
			def file = new File("src/test/groovy/org/groovykoans/koan11/cast.txt")
			file.eachLine { String line ->
				def vector = line.split(" ")
				db.execute("insert into PERSON (FIRSTNAME,LASTNAME) values ('${vector[0]}','${vector[1]}');")				
			}
			...
			xml.character.each{ c ->				
				def vector = c.@actor.text().split(" ")
				db.execute("insert into PERSON (FIRSTNAME,LASTNAME) values ('${vector[0]}','${vector[1]}');")			
			}			
