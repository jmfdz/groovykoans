...
class Koan10 extends GroovyTestCase {
    ...
    void test01_XmlSlurpersReader() {
        ...
        // ------------ START EDITING HERE ----------------------
		xml.movie.collect { 
			if(it.title.text().toLowerCase().contains('the') )
				moviesWithThe << it.title.text() 			
		}
    ...
    void test02_XmlSlurpersReader2() {
        ...
        // ------------ START EDITING HERE ----------------------
		def xml = new XmlSlurper().parse(new File("src/test/groovy/org/groovykoans/koan10/movies.xml"))
		def listaDeNodos = xml.movie.list().sort { a, b ->
        	def anoa = a.year.text().toInteger()
			def anob = b.year.text().toInteger()
			if(anoa == anob)
				return a.title.text() <=> b.title.text()
			else
				return anoa <=> anob			
		}
		sortedList = listaDeNodos.collect{ it.title.text() }
        // ------------ STOP EDITING HERE  ----------------------
    ...
    void test04_XmlMarkupBuilder2() {
		...
		def b = new MarkupBuilder(writer)
		b.movies {
			xml.movie.list().each{ m ->
				movie(id:m.@id.text(),title:m.title.text(),year:m.year.text())
			}
		}
    ...
    void test05_AntBuilderCopy() {
		...
		b.copy(file:"src/test/groovy/org/groovykoans/koan10/movies.xml", toFile:"src/test/groovy/org/groovykoans/koan10/movies_copy.xml")
    ...
    void test06_AntBuilderChecksum() {
        ...
        // ------------ START EDITING HERE ----------------------
		def b = new AntBuilder()
		b.checksum(file:"src/test/groovy/org/groovykoans/koan10/movies.xml", property: 'moviesChecksum')
        actualChecksum = b.project.properties.moviesChecksum
        // ------------ STOP EDITING HERE  ----------------------
